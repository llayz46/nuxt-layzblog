<script setup>
    import { useTableOfContentStore } from "~/stores/tableOfContentStore.js";
...
<template>
    <Transition
        ...
        <div v-if="storeToc.open" class="fixed top-0 right-0 mt-16 mr-3 w-80 max-[360px]:w-64 max-h-[calc(100%-5rem)] backdrop-blur-md bg-my-background/10 border border-white/10 rounded-xl p-5">
    ...
    <button v-if="!storeToc.open" @click="storeToc.toggle" class="fixed top-0 right-0 mt-16 mr-3 max-w-80 max-[360px]:max-w-64 backdrop-blur-md bg-my-background/10 border border-white/10 rounded-xl p-2">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 text-white/50 hover:text-white transition-colors">
